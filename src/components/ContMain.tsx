    ...
    <section className='flex mediaContMain flex-col items-center overflow-y-auto overflow-x-hidden rounded-t-3xl background-3 w-[46.8vw] min-w-[500px] border border-white border-opacity-10 h-[80svh]'>
        ...
        <section>
          ...
          {messages.map((message) => (
            <article key={message.id}>
              <PostS url={message.url} id={message.uid} user={message.displayName} contenido={message.contenido} displayName={message.displayName} like={message.like} image='' comment={message.comment} rePost={message.rePost} share={message.share} />
            </article>
          ))}
        </section>
      ) : (
      ...
      {activeT ? (
        <section className='w-full mt-3'>
          {messages.map((message) => (
            <article key={message.id}>
              <PostS url={message.url} user={message.user} id={message.uid} contenido={message.contenido} displayName={message.displayName} like={message.like} image='' comment={message.comment} rePost={message.rePost} share={message.share} />
            </article>
          ))}
        </section>
      ) : (
